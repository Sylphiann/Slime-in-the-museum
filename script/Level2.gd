...
func _ready() -> void:
	...
	Stats.current_level = "res://script/Level2.gd"
	Stats.can_move = true
	Stats.hp = 3
	Stats.energy = 0
	Stats.stage_one = stage_one
	Stats.stage_two = stage_two
	Stats.finish = finish
	pause.hide()
...
func _on_area_2d_body_entered(body: Node2D) -> void:
	if body.name == "Slime" and Stats.energy >= Stats.finish:
		...
		transition.play_transition(
			func(): get_tree().change_scene_to_file(str("res://scene/WinScreen.tscn"))
		)
